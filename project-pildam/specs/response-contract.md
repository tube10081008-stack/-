...
인사, 자기소개, "너 AI야?", 딴소리에 대한 응답은 **인용을 동반하지 않는다.**
문학적 소득을 주장하지 않는 턴이므로 VQR 하한을 적용할 대상이 아니며, 대신 짧아야 한다.

| 규칙 | 값 |
|---|---|
| 인용 | 없음 (있으면 그것은 응대 턴이 아니다) |
| 모델 저작 예산 | **140자** |
| 금칙 문체 | 독서 턴과 동일하게 전부 적용 |
| `ai` 의도 | AI가 생성한 응답임을 **시스템 알림으로 함께 고지**한다 |
| 권유 | 대목을 보자고 청했으면 같은 턴에서 실제로 연다. 지키지 않을 약속은 하지 않는다 |

응대 턴을 두지 않으면 "안녕?"에 소네트가 날아간다. 실제로 그랬다.

## 3-c. 위작 금칙 (counterfeit)

응대 턴은 모델이 자유롭게 문장을 쓰는 **유일한 자리**이고, 그래서 L1이 새는 자리다.
실측에서 모델은 곧바로 흉내 셰익스피어를 쓰기 시작했다 —
"인간의 삶은 덧없는 그림자입니다", "세상 만사는 한 편의 연극입니다".
작품을 인용하지 않았을 뿐 작가의 어조로 잠언을 위조한 것이므로 L1 위반이다.

- 런타임: 작가의 어조로 삶·세상·인간에 대한 일반 명제를 단정하는 문장은 금칙 문체로 차단한다.
- 정규식은 이 부류를 전부 잡지 못한다. 따라서 **응대 라인은 번역과 마찬가지로
  사람의 검수를 거친 것만 배포한다.** 자동 생성분을 그대로 싣지 않는다.
